...
author: "Andy Spendlove, Georgia Christodoulou, Adam Bushman"
date: "2024-10-31"
output: 
  html_document:
...
roc_obj1 <- pROC::roc(
  ifelse(imbal_data$DEFAULT, 1, 0), 
  ifelse(imbal_data$pred, 1, 0)
)
...
# Calculate summary statistics
roc_obj2 <- pROC::roc(
  ifelse(bal_data$DEFAULT == 'Y', 1, 0), 
  ifelse(bal_data$pred == 'Y', 1, 0)
)
...
Support Vector Machines are a logical choice for a high dimensional problem such as this. They do well to avoid overfitting and can handle high dimensions very well. 

However, they are computationally intensive and are better suited for situations where the number of observations is small. It particularly is good when the count of rows exceed the count of columns. That *is not* our situation here.

Still, it should provide a good alternative to tree-based and OLS type approaches.

### Model Preparation

Leveraging support vector machines in R using the `{e1071 }` package requires its own set of formatting requirements. Specifically, any column with a constant value must be removed. 

Additionally, SVM is computationally intensive so we absolutely cannot run the model on the entire 550K data set. We'll work just just 2% of the data (around 10K records). 

This is the flow for preparing the data for SVM in code format:
...
This latter code chunk dynamically removes any column that has a single, unique value (a constant). We then have a nice training and testing sample to work with.
...
```{r eval=FALSE}
roc_obj <- pROC::roc(ifelse(test_sampl$DEFAULT == "Y", 1, 0), ifelse(pred_probs == "Y", 1, 0))
...
We can see the results of this final model reading from that CSV:

```{r}
svm_results <- read.csv('models/support-vector-machine/model-results.csv')
t(svm_results)
```
