...
application_cleaning <- function(data, train = TRUE) {
  ...
  if(train) {
    ...
    data <- data[!(data$FLAG_OWN_CAR == "Y" & is.na(data$OWN_xCAR_AGE)), ]
  } else {
    # PRESERVE ROWS & IMPUTE IF THIS IS TESTING
    if(sum(is.na(data$OWN_xCAR_AGE)) > 0) {
      mean_own_car_age <- median(as.integer(data$OWN_xCAR_AGE), na.rm = TRUE)
      data$OWN_xCAR_AGE[!(data$FLAG_OWN_CAR == "Y" & is.na(data$OWN_xCAR_AGE))] <- mean_own_car_age
    }
  }
